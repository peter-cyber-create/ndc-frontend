...
export default function AbstractsPage() {
  ...
  useEffect(() => {
    loadAbstracts()
  }, [page, pageSize, statusFilter, categoryFilter, searchTerm])
  ...
  const loadAbstracts = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/admin/abstracts?${params.toString()}`, { cache: "no-store" })
      if (response.ok) {
  ...
  const handleDownloadAbstract = async (fileUrl: string, abstract: Abstract) => {
    try {
      const response = await fetch(`${API_URL}/api/abstracts/download/${abstract.id}?t=${Date.now()}`)
